...
class BaseH(KGModel):
    ...
    def get_c(self, queries):
        """Return curvature for each query in the batch."""
        if self.multi_c:
            return F.softplus(self.c[queries[:, 1]])
        return F.softplus(self.c).expand(queries.shape[0], 1)
