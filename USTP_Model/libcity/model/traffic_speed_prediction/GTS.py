...
class GCONV(nn.Module):
    ...
    def forward(self, inputs, state, adj_mx):
        # Perform graph convolution on X(t) and H(t-1) and add bias
        ...
        # 3rd order [T0, T1, T2] Chebyshev polynomial approximation g(theta)
        # Replace ~L in the graph convolution formula with random walk Laplacian D^(-1)*W
...
class EncoderModel(nn.Module, Seq2SeqAttrs):
    ...
    def forward(self, inputs, adj, hidden_state=None):
        ...
        for layer_num, dcgru_layer in enumerate(self.dcgru_layers):
            ...
            output = next_hidden_state  # cycle
...
class GTS(AbstractTrafficStateModel, Seq2SeqAttrs):
    def __init__(self, config, data_feature):
        ...
        # The role of adj_mx here is to serve as a reference when training automatic graph structure inference.
        self.adj_mx = torch.Tensor(data_feature.get('adj_mx')).to(self.device)
    ...
    def forward(self, batch, batches_seen=None):
        ...
        # The inference process of graph structure
    ...
    def calculate_loss(self, batch, batches_seen=None):
        ...
        # Selectively add regular terms according to the number of training rounds
