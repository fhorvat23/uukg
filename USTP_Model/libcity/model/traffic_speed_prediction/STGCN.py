...
def calculate_first_approx(weight):
    ...
    :return: np.ndarray
    '''
    # TODO: What if the W diagonal is all 1's?
    n = weight.shape[0]
...
class TemporalConvLayer(nn.Module):
    ...
    def _safe_conv(self, x):
        try:
            return self.conv(x)
        except RuntimeError as e:
            if "Unable to find a valid cuDNN algorithm" in str(e) and torch.backends.cudnn.enabled:
                return self._conv_without_cudnn(x)
            raise

    def _conv_without_cudnn(self, x):
    ...
    def forward(self, x):
        ...
        if self.act == "sigmoid":
            return torch.sigmoid(self._safe_conv(x) + x_in)  # residual connection
        return torch.relu(self._safe_conv(x) + x_in)  # residual connection
...
class STGCN(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        # Calculate the normalized Laplacian matrix of the GCN adjacency matrix and the corresponding Chebyshev polynomial or first-order approximation
        if self.graph_conv_type.lower() == 'chebconv':
            ...
            self.Lk = torch.FloatTensor(self.Lk).to(self.device)
            self.Ks = 1  # The first-order approximation is retained to K0 and K1, but it is not in array form, there is only an n*n matrix, so it is 1 (essentially 2)
        else:
