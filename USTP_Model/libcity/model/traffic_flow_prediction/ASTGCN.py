    ...
    def forward(self, x, spatial_attention):
        ...
        for time_step in range(num_of_timesteps):
            ...
            for k in range(self.K):
                ...
                t_k_with_at = t_k.mul(spatial_attention)   # (N,N)*(B,N,N) = (B,N,N) .mul->element-wise multiplication
...
class ASTGCNBlock(nn.Module):
    def __init__(self, device, in_channels, k, nb_chev_filter, nb_time_filter,
        ...
        # Time convolution: input time length = num_of_timesteps = time_strides * output_window
        # The input must be a fixed multiple of the output output_window!
        # ker=3, pad=2, stride=time_strides
        # Output time length = (time_strides * output_window + 2 * pad - ker) / time_strides + 1 = output_window
        self.time_conv = nn.Conv2d(nb_chev_filter, nb_time_filter, kernel_size=(1, 3),
                                   stride=(1, time_strides), padding=(0, 1))
        # Convolution in the time dimension: input time length = num_of_timesteps = time_strides * output_window
        # ker=1, stride=time_strides
        # Output time length = (time_strides * output_window - ker) / time_strides + 1 = output_window
        self.residual_conv = nn.Conv2d(in_channels, nb_time_filter, kernel_size=(1, 1), stride=(1, time_strides))
        self.ln = nn.LayerNorm(nb_time_filter)  # The channel needs to be placed in the last dimension
    ...
    def forward(self, x):
        ...
        # Combined with temporal attention: (B, N*F_in, T) * (B, T, T) -> (B, N*F_in, T) -> (B, N, F_in, T)

        # SAt
        spatial_at = self.SAt(x_tat)  # (B, N, N)

        # Graph convolution combined with spatial attention cheb gcn
        ...
        # (B, N, F_out, T) -> (B, F_out, N, T) Use the convolution kernel of (1,3) to do it -> (B, F_out', N, T') F_out'=nb_time_filter

        # residual shortcut
        x_residual = self.residual_conv(x.permute(0, 2, 1, 3))
        # (B, N, F_in, T) -> (B, F_in, N, T) Use the convolution kernel of (1,1) to do it -> (B, F_out', N, T') F_out'=nb_time_filter
...
class ASTGCNSubmodule(nn.Module):
    ...
    def forward(self, x):
        ...
        for block in self.BlockList:
            x = block(x)  # The output dimension of each space-time block is nb_time_filter
        # (B, N, F_out(nb_time_filter), T_out(output_window))
        # Change nb_time_filter to output_dim
...
class ASTGCN(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        if self.len_period == 0 and self.len_trend == 0 and self.len_closeness == 0:
            # Some forks use TrafficStatePointDataset without CPT fields in data_feature.
            # In that case, degrade to a closeness-only setup driven by input_window.
            self.len_closeness = config.get('len_closeness', config.get('input_window', 0))
    ...
    def forward(self, batch):
        ...
        # The order in the time dimension (the 1st dimension) is CPT, that is
