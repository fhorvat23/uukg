...
class GraphConvolution(Module):
    def __init__(self, in_features, out_features, device, bias=True):
        ...
        self.weight = Parameter(torch.FloatTensor(in_features, out_features).to(device))  # FloatTensorCreate tensor
        if bias:
...
class CONVGCNCommon(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        self.adj_mx = torch.tensor(self.data_feature.get('adj_mx'), device=self.device)  # adjacency matrix
