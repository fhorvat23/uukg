    ...
    def forward(self, x, spatial_attention):
        ...
        for time_step in range(num_of_timesteps):
            ...
            for k in range(self.K):
                ...
                t_k_with_at = t_k.mul(spatial_attention)   # (N,N)*(N,N) = (N,N) The sum of multiple rows is 1, normalized by column

                theta_k = self.Theta[k]  # (in_channel, out_channel)

                rhs = t_k_with_at.permute(0, 2, 1).matmul(graph_signal)
                # (N, N)(b, N, F_in) = (b, N, F_in) Because it is left multiplication, the sum of multiple rows is 1 and the sum of multiple columns is 1, that is, the sum of one row is 1, and left multiplication is performed
...
# Adapt to the most general TrafficStateGridDataset and TrafficStatePointDataset
