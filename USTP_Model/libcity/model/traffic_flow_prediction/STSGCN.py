...
# Spatiotemporal embedding matrix, the embedding representation of real spatiotemporal features
...
# Graph convolution, nothing to say
...
# Synchronous spatiotemporal convolution module captures the spatiotemporal features of 3 consecutive time slices
class Stsgcm(nn.Module):
    ...
    def forward(self, data, adj):
        # Multiple convolutional layers are superimposed,
        need_concat = []
        for i in range(len(self.layers)):
            data = self.layers[i](data, adj)
            need_concat.append(torch.transpose(data, 1, 0))

        # And the output of each convolutional layer is input to the aggregation layer in a form similar to a residual network.
        need_concat = [
            torch.unsqueeze(
...
class STSGCNLayerIndividual(nn.Module):
    def __init__(self, t, num_of_vertices, num_of_features, filters,
        ...
        # A GCM module can capture the spatiotemporal characteristics of 3 consecutive time slices
        # T time slices, one capture every 3, that is, T - 2 times, corresponding to T - 2 GCM modules
    ...
    def forward(self, data, adj):
        ...
        # Splice the output of individual GCM modules
        return torch.cat(need_concat, dim=1)
        # shape is (B, T-2, N, C')


# The author of the paper uses it for ablation experiments. The difference from Sthgcn_layer_individual is that it shares the same stsgcm module.
...
def construct_adj(a, steps):
    """Construct a local space-time diagram

    Parameters
    ----------
    a: np.ndarray, shape: (N, N), original image adjacency matrix

    steps: time step length, the original paper is 3 in one

    Returns
    ----------
    Local space-time graph matrix shape: (N * steps, N * steps)"""
    n = len(a)
    ...
    # In fact, it adds two adjacent time step nodes to its own edge.
    for i in range(n):
...
class STSGCN(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        # Enter the embedding layer to increase the number of features and improve representation capabilities.
        ...
        # Spatio-temporal synchronized convolutional layer group
        ...
        # Output layer, one fully connected layer for each prediction time step
