...
def get_logger(config, name=None):
    ...
    # Avoid duplicated logs when get_logger() is called repeatedly
    # in the same process (e.g., multiple runs in run.py).
    if logger.handlers:
...
def trans_naming_rule(origin, origin_rule, target_rule):
    ...
    Args:
        origin (str): variable name in origin naming format
        origin_rule (str): source naming format, enumeration class
        target_rule (str): target naming format, enumeration class

    Return:
        target (str): the result after conversion"""
    # TODO: Please ensure that the input complies with origin_rule. There is currently no check here.
    target = ''
