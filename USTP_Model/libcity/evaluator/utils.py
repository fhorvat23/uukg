...
def output(method, value, field):
    """Args:
        ...
        value: the evaluation result value corresponding to the evaluation method
        field: the scope of evaluation, for a trajectory or the entire model"""
    if method == 'ACC':
        if field == 'model':
            print('---- Model under {} metric avg_acc={:.3f} ----'.format(method,
                                                                  value))
        else:
            print('{} avg_acc={:.3f}'.format(method, value))
    elif method in ['MSE', 'RMSE', 'MAE', 'MAPE', 'MARE', 'SMAPE']:
        if field == 'model':
            print('---- Model under {} metric avg_loss={:.3f} ----'.format(method,
                                                                   value))
        else:
...
def transfer_data(data, model, maxk):
    ...
    assert type(data) == dict, "Data to evaluate must be a dict with valid structure"
    if model == 'DeepMove':
        user_idx = data.keys()
