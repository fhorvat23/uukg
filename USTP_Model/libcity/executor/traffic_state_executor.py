...
class TrafficStateExecutor(AbstractExecutor):
    ...
    def _build_optimizer(self):
        """Select optimizer based on global parameter `learner`"""
    ...
    def _build_lr_scheduler(self):
        """Select the corresponding lr_scheduler according to the global parameter `lr_scheduler`"""
        if self.lr_decay:
    ...
    def _build_train_loss(self):
        """Select the loss function of the training process according to the global parameter `train_loss`
        If this parameter is none, you need to use the model's custom loss function.
        Note that the loss function should receive the `Batch` object as input and return the corresponding loss (torch.tensor)"""
    ...
    def train(self, train_dataloader, eval_dataloader):
        ...
        for epoch_idx in range(self._epoch_num, self.epochs):
            ...
            if self.hyper_tune:
                if tune is None:
                    raise ModuleNotFoundError('ray is required for hyperparameter tuning. Install ray[tune].')
    ...
    def _train_epoch(self, train_dataloader, epoch_idx, loss_func=None):
        """Complete one round of model training

        Args:
            train_dataloader: training data
            epoch_idx: number of epochs
            loss_func: loss function

        Returns:
            list: array of losses for each batch"""
        self.model.train()
    ...
    def _valid_epoch(self, eval_dataloader, epoch_idx, loss_func=None):
        """Complete one round of model evaluation
        ...
        Returns:
            float: the average loss value of the evaluation data"""
        with torch.no_grad():
            self.model.eval()
            loss_func = loss_func if loss_func is not None else self.model.calculate_loss
            losses = []
